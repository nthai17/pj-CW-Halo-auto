...
const Cart = () => {
  ...
  if (products !== null && products.length > 0) {
    itemsPrice = products.reduce(
      (total, item) => total + item.currentPrice * item.amount,
      0
    );
  } else {
    return (
      <>
        <BreadCrumb
          items={[{ text: "Trang chủ", path: "/" }]}
          last="Giỏ hàng"
        />
        <div className="grid wide home__section__wrap mt-3 mb-3">
          Không có sản phẩn nào, quay lại cửa hàng để mua sắm.
          <div>
            <Link to="/">
              <div className="payment__continue mt-5">tiếp tục mua hàng</div>
            </Link>
          </div>
        </div>
      </>
  ...
  return (
    <>
      <BreadCrumb items={[{ text: "Trang chủ", path: "/" }]} last="Giỏ hàng" />
      <section className="grid wide home__section__wrap">
        ...
        <div className="payment">
          <Link to="/">
            <div className="payment__continue">tiếp tục mua hàng</div>
          </Link>
